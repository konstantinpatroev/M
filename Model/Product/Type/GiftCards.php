    ...
    protected function _prepareProduct(\Magento\Framework\DataObject $buyRequest, $product, $processMode)
    {
        ...
        if(isset($data['card_amount'])) {
            if ($product->getMageworxGcAllowOpenAmount() && $data['card_amount'] == 'other_amount') {
                $isMin = $product->getMageworxGcOpenAmountMin() > 0 && $data['card_amount_other'] < $product->getMageworxGcOpenAmountMin();
                $isMax = $product->getMageworxGcOpenAmountMax() > 0 && $data['card_amount_other'] > $product->getMageworxGcOpenAmountMax();

                if ($isMin || $isMax) {
                    return $this->getSpecifyPriceMessage()->render();
                }
                $product->setData('price', $data['card_amount_other']);
                $data['card_amount'] = $data['card_amount_other'];
            } else {
